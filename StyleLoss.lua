...
function StyleLoss:__init(strength, loss_type, agg_type, shift)
  parent.__init(self)
  ...
  if self.agg_type == 'gram' then
    self.agg = nn.GramMatrix(true, self.shift)
...
function StyleLoss:updateOutput(input)
  ...
  elseif self.mode == 'loss' then
    ...
    self.loss = self.strength * self.crit:forward(self.agg_out, target)
    self._target = target
  end
  self.output = input
  return self.output
...
function StyleLoss:setStrength(strength)
